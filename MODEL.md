...
denominator and NaN for 0/0, but the `ratioAbove`/`ratioBelow` comparators **require a
finite value** — a non-finite ratio (both x/0 → +inf and 0/0 → NaN) FAILS the assertion
loudly, so a degenerate zero denominator can never silently satisfy `ratioAbove`;
