...
| T2 | *(created)* | `blocked` | Team sim / org sim with AI active: a **brittleness event** fires on a novel arrival (§4 M9/M10). |
...
| T4 | `queued(L)` | `inProgress` | Service time at final layer `L` elapsed and capacity available. The task's `age` at this instant is recorded as a **decision-latency sample**. |
| T5 | `inProgress` | `inProgress` | Execution allocates points; `progress += allocation` (§5 step 7). |
| T6 | `inProgress` | `done` | `progress ≥ effort`. Counted in `throughput` this step. Team sim: passes through a review dwell (`reviewDwellDays`) before `done` when the review function is covered. |
...
The org simulator uses this FSM with `L = ownershipLayers` and no review dwell
...
the Prioritization function (§4 M11).
...
{
  "id": "overrideWipReset",
  "formula": "Per step, each inProgress task is overridden with probability o(t) = overrideBaseRate * (L - 1) * (1 + overrideMisalignmentGain * m(t)) * (1 + distortionOverrideCoupling * distortion); distortion = distortionPerHumanLayer * (D - 1). On override: task returns to layer 1, progress *= wipResetFraction.",
...
{
  "id": "cohesionDynamics",
  "formula": "Per team: target(t) = cohesionBase - cohesionSizePenalty * sigma((s - cognitiveBandClose)/(bandWidthFactor * cognitiveBandClose)) - cohesionAiPenalty * effectiveAiShare - (hollow ? cohesionHollownessPenalty : 0). c(t+1) = clamp(c(t) + cohesionRecoveryRate * (target - c(t)) - cohesionEntropyCoupling * max(0, E(t) - entropyStressThreshold) / 10, 0, 100). c(0) = cohesionBase. Team sim: effectiveAiShare = (# AI entities)/(# entities); hollow = for any of {prioritization, coordination, stakeholderCommunication}, at least one AI entity is assigned AND no human entity is assigned. Org sim: effectiveAiShare = aiActive ? aiRoutineShareOrg * taskMixRoutineOrg : 0; hollow = false.",
...
to more override draws. **Boundedness:** override probability per task-step is a constant
for a given config (≤ `overrideBaseRate × 4 × (1+overrideMisalignmentGain) ×
(1+distortionOverrideCoupling×0.4) < 0.5`), progress loss is bounded by
`wipResetFraction`, and layer capacity caps pipeline flow — the loop amplifies latency
but cannot diverge: latency is bounded by queue length, which is bounded by cumulative
arrivals (at most linear growth in t in overloaded configs).
...
composite (M13 cohesion term). This is a genuine reinforcing loop. **Boundedness:** both
quantities are clamped to [0,100]; cohesion is pulled toward a finite structural target
at rate `cohesionRecoveryRate`; the coupling drain is bounded by
`cohesionEntropyCoupling × 4` points/step. The pair settles at an interior fixed point
whenever the coupling drain at max entropy is smaller than the recovery pull at the
target gap — true for all defaults and all values within the declared ranges.
...
| `meetingOverheadPct` | fraction 0–1 | M4b; render as % of capacity |
...
**Org non-series blocks:** `perLayer` — for each layer l: `{layer, meanLatencyDays,
meanQueue, utilization, overrideShare, distortion, bottleneck}` (means over the final 20
steps; `bottleneck` = layer with max utilization; `overrideShare` = fraction of override
events attributed to decisions above layer l−1, reported as the per-layer share of total
overrides; `distortion = distortionPerHumanLayer × (l−1)` for human layers);
...
Post-lock, values change only doc-first through the orchestrator (PLAN.md standing
rules): within-range default changes bump the minor modelVersion; range changes or new
parameters require a mini-G2 user gate.
...
{ "id": "aiCoordinationRelief", "value": 0.35, "range": [0.05, 0.35], "distribution": "point", "unit": "fraction of tax",
  ...
  "plainLanguage": "In a healthy org, AI trims up to a quarter of the coordination tax. In an unhealthy one: nothing." }
```
...
{ "id": "cohesionAiPenalty", "value": 15, "range": [10, 30], "distribution": "point", "unit": "index points at 100% AI",
  ...
  "plainLanguage": "The more teammates are agents, the weaker the human bonds get - about 20 points if the whole team were AI." }
```
...
The five stress scenarios are defined here; `www/presets/*.json` (P4/P7a) materialize
...
{ "id": "fdSeparability", "scenario": "fasterDysfunction",
  ...
  "rationale": "Pre-mortem A3: the launch-centerpiece chart must be visually unambiguous, golden-asserted rather than hoped for." }
```
...
{ "id": "hmEarlyThroughputBoost", "scenario": "hollowMiddle",
  ...
  "predicate": "By step 10 the hollow team has shipped at least 15% more items - work accelerates initially.",
  "bound": 1.15, "tolerance": 0.05, "step": 10, "instrument": "meanPath",
  "rationale": "The seductive first fortnight, before the edge cases arrive." }
```
...
- `schemaVersion` and `modelVersion` appear in **both** config and output.
  **modelVersion is declared in this file's meta block (§0) and emitted into
  `model/params.json` by the extractor** — the CI pairing gate anchors to that
  declaration (see §12.6).
- `validate()` rejects: unknown fields, NaN/Inf anywhere, out-of-range values,
  `paramOverrides` keys not present in `model/params.json`, override values (or
  triangular modes) outside the parameter's declared `range`, mix fractions not summing
  to 1 ± 0.001, and `cost.enabled == true` (v1).
...
```json
{
  ...
  "perLayer": [ { "layer": 1, "meanLatencyDays": 2.1, "meanQueue": 0.4, "utilization": 0.45, "overrideShare": 0.5, "distortion": 0.0, "bottleneck": false } ],
...
- **Evolution caveat (stated policy):** a mini-G2 amendment that adds a NEW parameter
...
pre-reserved v2 hooks. Unknown **major** schema versions are rejected gracefully by the
UI with an upgrade message.

### 12.6 modelVersion policy and CI pairing (declaration format)

`modelVersion` is MAJOR.MINOR.PATCH:

- **PATCH** — prose-only edits; `model/params.json` byte-identical.
- **MINOR** — any change to `model/params.json` (coefficient defaults within range,
  golden/tolerance changes via mini-G2, new parameters via mini-G2).
- **MAJOR** — mechanic formula changes, schema-affecting changes, RNG scheme/library
  major changes.

**Every amendment that changes `model/params.json` bumps modelVersion (at least MINOR)**
and appends a §14 changelog row. **CI pairing gate (P3 wires; P1 defines):** CI
recomputes `sha256(model/params.json)` and fails unless the §14 changelog table contains
...
- **industry-report:** 9 (`meetingHeavyMultiplier`, `decisionLatencyPerLayerDays`, the
  four cognitive band centers, `cohesionAiPenalty`, `teamPodTargetSize`,
  `hierarchicalTeamSize`).
- **editorial-heuristic:** all remaining parameters — adjustable defaults chosen so the
  mechanics behave plausibly, exposed via `paramOverrides` (UI sliders in v2).
...
7. **No cost/salary layer in v1** (reserved `cost` block; deliberate — see VISION
   non-goals). No mid-run restructuring events, hiring events, or attrition in v1.
