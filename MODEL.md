...
| T7 | `inProgress` | `queued(1)` | **Override event** (§4 M8): task re-enters layer 1 **ready — capacity wait only, NO fresh service draw** — `progress ×= wipResetFraction`, `overrideCount += 1`. It pays fresh per-layer service draws only as it re-advances through layers 2..L (T3). This pins the RNG draw stream (§8.1) for P4/P7a: an override consumes exactly its M8 Bernoulli plus the one M19 attribution uniform, and no layer-1 service draw. |
...
```json eigenorg:mechanic
{
  ...
  "formula": "A brittleness event draws ONE duration d ~ Triangular(recoveryDurationUnownedSteps | recoveryDurationOwnedSteps), rounded to whole steps (min 1), shared by the task block and the recovery window (its exact active steps are the INTERVAL SEMANTICS AS IMPLEMENTED clause below). While any window is active, new decision-service draws are multiplied by M_recovery(t) = max over active windows of their multiplier. Because M_recovery(t) is computed at step start (§5 step 2) BEFORE that step's own arrivals and unblocks, a window opened during t_event's arrivals (§5 step 3) first multiplies service draws at t_event+1; the event's own blocked task used the pre-event M_recovery. INTERVAL SEMANTICS AS IMPLEMENTED: for an event at step e with duration d, M_recovery multiplies service draws at steps e+1..e+d inclusive (the window is read at step start with its counter still positive, then decremented, so it first bites at e+1 and last at e+d); the paired blocked task's counter is decremented once per step starting the SAME step e it opened (§5 step 4) and the task unblocks (T8) during step e+d-1. Unowned recovery (org: SH < recoveryOwnershipThreshold; team: recoveryOwner == null): multiplier ~ Triangular(recoveryLatencyMultiplierUnowned). Owned: multiplier = recoveryLatencyMultiplierOwned (point value, no draw).",
...
```json eigenorg:mechanic
{
  ...
  "limitations": [
    ...
    "reviewWaitDays reports the integer realized review sojourn ceil(reviewDwellDays) (the M20 fractional-dwell pin: the sojourn is the integer clear-entry step gap, so a fractional reviewDwellDays is realized as its ceiling) and lags the instantaneous queue by the EMA smoothing.",
...
- **Settling:** for static configs (no growth, no AI injection), the **continuous
  composite series the property test binds — `entropy` and `cohesionTeamAvg`** (and
  `orgHealth = 100 − entropy`) — reach a steady band (trailing-30-step p50 range < 10%
  of the trailing mean) by step 100 and stay there through 5× horizon. **Sparse integer /
  event series are exempt from this ratio criterion:** `throughput`, `overrideRate`,
  `brittlenessRate`, and `wip` take small integer values with a near-zero mean, so
  "range < 10% of mean" is ill-defined for them — they are held only to the
  boundedness / non-negativity contract above (and `wip`, plus metrics derived from it,
  may grow at most linearly in overloaded configs).
...
**Cross-iteration aggregation (Monte Carlo), as implemented.** The per-layer rules
above define one iteration's value; the runner reduces the 500 iterations per field as
follows. `meanLatencyDays`, `meanQueue`, and `utilization` are the **mean across
iterations of each iteration's own per-layer mean** (each iteration contributes its
final-20-step mean, using that iteration's structural-minimum fallback when no task
entered the layer in its window). `overrideShare` is computed from **pooled event
counts across all iterations** — Σ(override events attributed to layer l) / Σ(total
override events) — not a mean of per-iteration shares (so an iteration with no overrides
contributes nothing rather than a 0/0), and is 0 when no override occurred in any
iteration. `bottleneck` is the layer with the maximum cross-iteration mean
`utilization`. `layerType`, `distortion`, `ownerMultiplicity`, and `diffusionFactor`
are deterministic per config and identical across iterations.
...
| `reviewWaitDays` | working days | EMA realized review sojourn (M20); equals `ceil(reviewDwellDays)` while review capacity is unbounded (integer clear-entry gap; = `reviewDwellDays` at the default dwell 1), rises when it binds |
...
```json eigenorg:parameter
{ "id": "brittlenessNormPerStep", "value": 0.35, "range": [0.2, 1.0], "distribution": "point", "unit": "events/step",
  "anchor": "Editorial", "tier": "editorial-heuristic",
  "limitation": "Normalization constant.",
  "formula": "xBrit = min(1, brittleEMA / brittlenessNormPerStep) (M13)",
  "plainLanguage": "A failure roughly every three working days counts as maximal brittleness." }
```
...
A 7-entity team with **five AI execution agents** driving a steady completion stream into a
single human review function, run twice. `bottleneck` caps review clearance at
`reviewCapacityPerStep 1` (one review cleared per step — below the ~1.46 completions/step the
team delivers into review); `unbounded` leaves `reviewCapacityPerStep null` (the default —
unbounded parallelism, reproducing v1 review behaviour exactly). Only
`team.reviewCapacityPerStep` differs. This exercises the review capacity gate (M20): once the
producers (five AI execution agents) outrun the single human reviewer, done-throughput plateaus
at the reviewer's rate (Goldratt ToC) while the review queue grows and wait diverges (Little's
Law), regardless of how fast the AI agents complete work upstream. (The reviewer's clearance
rate, not the raw AI completion rate, sets the plateau: the calibrated completion-into-review
rate is ~1.46/step — the all-AI team is itself execution-limited on the novel-task minority —
so the review cap is set to 1/step to sit clearly below it and make the M20 gate binding.)
...
| `growthRatioAbove` | value(step[1]) / value(step[0]) ≥ bound × (1 − tol); a non-finite ratio (x/0 → +inf or 0/0 → NaN) FAILS loudly | number; step = [from, to] |
| `twoWindowRatioAbove` | mean(metric over windowA) / mean(over windowB) ≥ minRatio × (1 − tol); a non-finite ratio (x/0 → +inf or 0/0 → NaN) FAILS loudly | {windowA, windowB, minRatio} |
...
ratio convention: the ratio helper still forms +infinity for a positive-numerator zero
denominator and NaN for 0/0, but the ratio-family comparators — `ratioAbove`,
`ratioBelow`, `growthRatioAbove`, and `twoWindowRatioAbove` — **require a finite
value** — a non-finite ratio (both x/0 → +inf and 0/0 → NaN) FAILS the assertion
loudly, so a degenerate zero denominator can never silently satisfy any of them;
...
{ "id": "hmBrittlenessSpike", "scenario": "hollowMiddle",
  "metric": "cumulativeBrittleness@hollow - cumulativeBrittleness@humanPm", "comparator": "above",
  "predicate": "By the end, the hollow team has logged at least 1.5 more cumulative brittleness events than the human-PM team.",
  "bound": 1.5, "tolerance": 0.1, "step": null, "instrument": "meanPath",
  "rationale": "Novel decisions hitting an AI router without human judgment is the failure mechanism (red-team stress test 2). Re-anchored (mini-G2 #1, USER-APPROVED 2026-07-07) from the pointwise RATIO cumulativeBrittleness@hollow over cumulativeBrittleness@humanPm (ratioAbove 2) to the pointwise DIFFERENCE hollow minus humanPm (above 1.5) at the final step: the ratio was structurally unpassable on the engine because the human-PM team's cumulative novel failures (humanNovelFailureBase) are legitimately 0 at every probed seed, so the ratio is +infinity and the P3 §11.1 hardening makes every ratio-family comparator FAIL loudly on a non-finite value - a direct collision between two locked clauses (the v1 '+inf satisfies ratioAbove' convention this rationale used to rely on, and the P3 non-finite-fails addendum). The finite difference preserves the model claim (the hollow team suffers materially more brittleness) and cannot be non-finite. Measured hollow minus humanPm = 2.000 at the final step on all five probe seeds {42, 7, 123, 999, 2024} (500 iterations each), clearing the raw bound 1.5 by 33% on every seed. Same twin mechanism, different operator - the identical re-anchor pattern the user approved for hmEarlyThroughputBoost (single step to window mean) and rbThroughputPlateau (per-step to cumulative ratio). The paired hmBrittlenessFloorMc still pins the hollow count to >= 1.5, so the difference is carried by a strictly positive hollow numerator, not a vanishing human denominator." }
```
...
{ "id": "adCommitteeDiffusionCost", "scenario": "committeeInversion",
  ...
  "rationale": "M19 additive diffusion: committeeDiffuse's layerOwnerCount 3 on the middle seat raises diffusionMean = mean over l in 2..=L of (1 + overrideDiffusionGain*(mu_l-1)) to ~1.4 (one of two averaged seats at mu=3), scaling the M8 override probability; committeeSingle (layerOwnerCount 1) is the v1 committee with diffusionMean = 1. Both runs share the identical committee §9.9 relay factors, so the ratio isolates the diffusion cost. (Calibration-confirmed against the engine golden harness, seed 42, 500 iters, 2026-07-04: measured 1.25 (25/20 cumulative overrides); the bound 1.15 is kept predicate-consistent — cleared to 1.035 effective by the 0.1 tolerance, absorbing the ±1-event lumpiness of the ~20-count ratio.)" }
```
...
{ "id": "rbQueueBuilds", "scenario": "reviewBottleneck",
  ...
  "rationale": "M20 capacity gate: the producers (five AI execution agents) deliver a ~1.46 completions/step stream into review — the all-AI team is itself execution-limited on the novel-task minority, so raw AI speed does not all reach review — outrunning the single human reviewer (reviewCapacityPerStep 1, one clearance/step), so the queue grows ~linearly at (completionRate − reviewCapacity) ≈ 0.46/step (Goldratt ToC: surplus upstream of the constraint accumulates as WIP). Window starts at step 10 to skip warm-up transients. (Calibration-confirmed against the rebuilt scoped team calibrator, seed 42, 500 iters, 2026-07-04: measured queue rise ≈22 over [10,59]; the bound 10 is kept predicate-consistent and cleared with a wide margin.)" }
```

```json eigenorg:golden
{ "id": "rbThroughputPlateau", "scenario": "reviewBottleneck",
  "metric": "cumThroughput@bottleneck / cumThroughput@unbounded", "comparator": "ratioBelow",
  "predicate": "By the end of the run the capped team has shipped at most 0.85x the cumulative done-throughput of the uncapped team - done-throughput plateaus at the reviewer's rate no matter how fast the AI agents complete work.",
  "bound": 0.85, "tolerance": 0.05, "step": null, "instrument": "meanPath",
  "rationale": "The Theory-of-Constraints invariant (Goldratt 'The Goal'): system throughput-to-done is capped by the slowest stage. Bottleneck done-rate ≈ reviewCapacity (1/step) while the unbounded control clears the full ~1.46/step completion stream, so the capped team's cumulative done-throughput ends well below the uncapped control's. Raising producer speed upstream cannot lift the plateau; only elevating the constraint can. METRIC BASIS (mini-G2 approved 2026-07-04): the metric is the ratio of the cumulative `cumThroughput` series (the §7.2 running total of completions, already the carrier for hmEarlyThroughputBoost) at the final step, NOT the per-step `throughput` count. At the reviewer's ~1/step rate the per-step `throughput` is a small integer whose p50 cannot distinguish 1 from 1.46 (it reads 0.95, masking the plateau); the cumulative total is a large integer (bottleneck ≈58 vs unbounded ≈85 over 60 steps) whose p50 ratio robustly measures the plateau. Calibration-confirmed against the rebuilt scoped team calibrator, seed 42, 500 iters, 2026-07-04: measured 0.6824, which the 0.85 ratioBelow bound (kept predicate-consistent) clears with ~20% margin. cumThroughput is an existing team output series (no new series / schema change); referencing it mirrors hmEarlyThroughputBoost." }
```
...
{ "id": "rbWaitRises", "scenario": "reviewBottleneck",
  ...
  "rationale": "Little's Law (W = L / throughput): with a growing queue L cleared at the fixed reviewCapacity (1/step), review wait rises superlinearly (M/M/1 rho/(1−rho) as rho → 1). Anchored well below the tens-of-days the divergent queue implies, and far above the well-run few-hours latency (Sadowski et al. 2018) the unbounded control tracks. (Calibration-confirmed against the rebuilt scoped team calibrator, seed 42, 500 iters, 2026-07-04: measured final review wait ≈19.3 working days; the bound 4 is kept predicate-consistent and cleared with a wide margin.)" }
```
...
{ "id": "rbControlBounded", "scenario": "reviewBottleneck",
  ...
  "rationale": "The neutral-identity anchor for M20: null capacity means every dwell-elapsed completion clears each step, so the queue never accumulates a runaway backlog and done-throughput is not gated - the v1 behaviour. reviewQueueDepth holds only the in-dwell population (≈ completionRate × reviewDwellDays ≈ 1.46). (Calibration-confirmed against the rebuilt scoped team calibrator, seed 42, 500 iters, 2026-07-04: measured unbounded final queue ≈1 (p50); the bound 5 is kept predicate-consistent, comfortably above the small in-dwell population.)" }
```
...
{ "id": "rbQueueFloorMc", "scenario": "reviewBottleneck",
  ...
  "rationale": "Band-level check that the review queue builds across seeds (mirrors ccEntropyFloorMc). rho ≥ 1 makes the queue grow deterministically in expectation, so even the p10 path accumulates a large backlog over 60 steps. (Calibration-confirmed against the rebuilt scoped team calibrator, seed 42, 500 iters, 2026-07-04: measured p10 final queue ≈14; the bound 10 is kept predicate-consistent and cleared with margin.)" }
```
...
```json
{
  ...
  "perLayer": [ { "layer": 1, "layerType": "humanPm", "meanLatencyDays": 2.1, "meanQueue": 0.4, "utilization": 0.45, "overrideShare": 0.0, "distortion": 0.0, "ownerMultiplicity": 1.0, "diffusionFactor": 1.0, "bottleneck": false } ],
...
- Serialized size stays **< 200 KB for the shipped scenarios and presets**
  (horizon ≤ 120 — the heaviest committed config, `coordinationCollapse` /
  `dunbarCliff` — which is what P3 asserts against the horizon-120 fixture). This
  200 KB figure is a **product constraint on shipped content, not a schema bound**:
  a schema-max horizon-600 config carries ~5× the per-step points and scales to
  ~0.6–1 MB. Keeping shipped presets at horizon ≤ 120 keeps every share-URL payload
  inside the budget; a hand-authored horizon-600 config is out of that budget's
  scope by design.
...
| 2.0.0 | 2026-07-04 | `60e0ebd0c51b54562b585cf2adf7bc4c602113e23535be201265c5b541440b82` | `6ce58c27b7c1dcc4f12af7b76e943eb4b5cc2b7687061d2c9c8249998f000b06` | `2972d98c9b4817ed0ddf92dd93c911be5beb5d72a78ba6432be215a818f081c5` | Accountability multiplicity μ (§4 M19 `accountabilityDiffusion`) + review-capacity queue (§4 M20 `reviewCapacityQueue`). M8 gains a diffusion term (μ>1 co-equal owners raise relitigation) and an opt-in authority-gradient override attribution (default `0.0` = v1 uniform draw); four new params in **§9.10** (all editorial-heuristic; document order fixes params.json order/SHA). New additive-optional config fields `org.layerOwnerCount` (int 1–8), `org.matrix{enabled,tiebreaker}` (terminal-layer μ=2), `team.reviewCapacityPerStep` (config field, not a params.json coefficient); new team output series `reviewQueueDepth`, `reviewWaitDays` (additive; consumers ignore unknown output fields). Four new scenarios (§10.7–§10.10) and 18 new goldens ⇒ **ten scenarios, 52 golden assertions** (34 v1 unchanged + 18 new). **Neutral identity (fully additive):** μ=1 / matrix off / unbounded review / gradient 0 reproduces v1.0.0 series byte-for-byte AND all 34 v1 golden verdicts for **every** pre-existing config, **including any using a `committee` seat** (a committee's §9.9 factors are unchanged and its default μ is 1; its accountability diffusion is now the opt-in, additive `org.layerOwnerCount` cost, so no existing `layerTypes` value changes meaning). `schemaVersion` stays `"1"`. **Calibration (2026-07-04):** the org-side v2 golden bounds (§11.8 accountabilityDiffusion, §11.9 committeeInversion, §11.10 matrix) were calibration-confirmed against the engine golden harness at seed 42 / 500 iterations — kept predicate-consistent and cleared with margin (the tolerance is the noise cushion) — and are asserted green; the `matrix` `mxTiebreakerRecovers` exact identity and the §11.6 `hmReviewWaitNeutral` identity both verify. **Mini-G2 amendment (approved 2026-07-04):** §10.10 `reviewBottleneck` `team.reviewCapacityPerStep` 2 → 1 so the single reviewer (1/step) sits below the ~1.46/step completion-into-review stream and the M20 gate is the binding constraint. Against the rebuilt scoped team calibrator, all five §11.11 goldens are calibration-confirmed green (queue rises ≈22, review wait ≈19 days). `rbThroughputPlateau`'s metric was changed (mini-G2 approved 2026-07-04) from the per-step `throughput` count to `cumThroughput@bottleneck / cumThroughput@unbounded` at the final step: the per-step `throughput` p50 cannot resolve the plateau at the reviewer's ~1/step rate (it reads 0.95), but the cumulative total does (bottleneck ≈58 vs unbounded ≈85, ratio 0.68, clearing the 0.85 bound with margin); `cumThroughput` is the existing §7.2 series that already carries hmEarlyThroughputBoost, so no new output series or schema change. All 52 v2.0.0 goldens are calibrated. **P3-review pre-lock addendum (2026-07-04, documentation-only):** the share-URL replay mechanism is now an explicit `config.replay` flag rather than a paramOverrides-cardinality inference (mechanism change only — the reproduction promise and every replayed number are unchanged); the §12.3 "<200 KB" figure is scoped to the shipped scenarios/presets (horizon ≤ 120), with a schema-max horizon-600 config disclosed as ~0.6–1 MB and out of that budget's scope; the §11.1 ratio-family comparators (`ratioAbove`, `ratioBelow`, `growthRatioAbove`, `twoWindowRatioAbove`) now fail loudly on a non-finite ratio (a +inf zero-denominator no longer silently satisfies any of them); and clarifying sentences were added to §2.2/M8 (an overridden task re-enters layer 1 ready with no fresh layer-1 service draw), M10 (recovery multiplier interval e+1..e+d; paired block unblocks at e+d−1), §7.1 (per-layer cross-iteration aggregation), and §6 (the settling contract binds the continuous `entropy`/`cohesionTeamAvg` series; sparse integer series exempt). No numeric, predicate, or bound change: params.json and goldens.json SHAs are unchanged; only the assumptions.json SHA moves, from the M8/M10 mechanic-prose clarifications. |
| 2.1.0 | 2026-07-05 | `6a8ec375442a44cd54c53b582ddd3c55dc887c652922eaf30cfd42fed1eea0ea` | `836f183b66fd96f18ef1540c0d2d8edfdea0103ce09b0978bc96bddd0c6aecc4` | `b006f23fdb8baedc5aa53f2d8b71380f04d47781eb67157dfa8afadda8967a7c` | **Within-range coefficient amendment (P4 engine calibration):** `brittlenessNormPerStep` 0.5 → 0.35 (range [0.2, 1.0] unchanged; editorial-heuristic normalization constant, M13 `xBrit` only). The real engine's §10.3 `fasterDysfunction` run produces ≈0.27 brittleness events/step at SH 3 post-injection — at the 0.5 norm the entropy composite registered a 3.80-point rise against the locked §11.4 `fdEntropyWorsens` predicate (≥ 5, tol 0.15): the only misfit in the 21 P4-landed org assertions (pt 6/6, dc 5/5, lc 4/4, fd 5/6 passed at defaults; M10 interval semantics verified against the pinned e+1..e+d clause first, per the P3 round-2 note). Norm sweep at seed 42: 0.45 → 4.50, 0.40 → 5.53, 0.35 → 6.63. 0.40 was rejected on multi-seed robustness: across seeds {42, 7, 123, 999, 2024} (500 iterations each, same §10.3 sh3 config, generic-evaluator code path) the rise is {5.53, 5.48, 4.91, 6.26, 6.52} — seed 123 falls below the raw bound 5, so the seed-42 margin is not seed-robust. At 0.35 (a failure roughly every three working days = maximal brittleness) the same five seeds give {6.63, 6.70, 6.09, 7.46, 7.94} — minimum 6.09 at seed 123, clearing the RAW bound by ≥ 21.7% on every probed seed, keeping the P3c convention that the tolerance is a noise cushion, never load-bearing. The change touches only runs with brittleness events: every zero-brittleness scenario series (coordinationCollapse, prioritizationTax, dunbarCliff, accountabilityDiffusion, committeeInversion, matrix) is byte-identical; the other five §11.4 verdicts stay green at seed 42 (fdSeparability 8.97 → 7.21, fdWorseThanNothing 12.01 → 14.96, fdEntropyImprovesHealthy 8.55 → 7.95; fdBrittlenessAmplified 6.00 and fdSeductiveThroughput 1.083 do not involve the norm); no §11 predicate, bound, or tolerance changed. No locked team golden asserts `entropyProxy`, so no calibrator-proven team bound is touched (the team-side effect at P7a is confined to the M12 cohesion coupling via `E_team`, in the direction that widens `hmCohesionErosion`). No schema, mechanic-formula, or RNG change ⇒ MINOR. |
| 2.2.0 | 2026-07-07 | `2a0b66ce170bbcc99a335fd4052f3bdb66df4a57968f8b822992a25f2968266e` | `bf649b85bcbd4a6cc2bcfdeaf148aaf4551581e0cb0c3d2156b257ba7fbc6d36` | `ac77cc81c66ef2f82bad9714a6935b04b011a640d403b022637dbf52ed1698eb` | **Two mini-G2 amendments (USER-APPROVED 2026-07-07; decision-log "AUTONOMY WINDOW #2").** **(1) `hmBrittlenessSpike` golden re-anchored (§11.6):** metric changed from the pointwise ratio `cumulativeBrittleness@hollow / cumulativeBrittleness@humanPm` (`ratioAbove`, bound 2) to the pointwise difference `cumulativeBrittleness@hollow - cumulativeBrittleness@humanPm` (`above`, bound 1.5, tol 0.1, final step, meanPath). The ratio was structurally unpassable on the real engine: the human-PM team's cumulative novel failures are legitimately 0 at every probed seed (λ≈0.54; humanNovelFailureBase sparse), so the ratio is +infinity, and the P3 §11.1 hardening fails every ratio-family comparator loudly on a non-finite value — a collision between two locked clauses. The finite difference preserves the model claim (the hollow team suffers materially more brittleness). Measured `hollow − humanPm = 2.000` at the final step on all five probe seeds {42, 7, 123, 999, 2024} (500 iterations each), clearing the raw bound 1.5 by 33% on every seed. Same twin mechanism, different operator — the identical re-anchor pattern approved for `hmEarlyThroughputBoost` (step→window) and `rbThroughputPlateau` (per-step→cumulative). This is the only goldens.json content change; it flips `hmBrittlenessSpike` from surfaced-contradiction to green, completing hollowMiddle 9/9 and the launch stress suite 5/5. **(2) M20 fractional-dwell text pin (§4 `reviewCapacityQueue`, documentation-only):** the realized review sojourn is pinned to the integer `clear step − entry step` gap = `ceil(reviewDwellDays)` at unbounded capacity, and the neutral-identity clause now reads `reviewWaitDays == ceil(reviewDwellDays)` (resolving the mini-G2 #2 fractional contradiction between M20's integer-sojourn operational definition and its `== reviewDwellDays` identity clause). No code, golden, range, or coefficient change; every shipped config uses `reviewDwellDays = 1` where `ceil(1) = 1`, so behavior is **byte-identical** and `hmReviewWaitNeutral` stays exactly [1,1]. The M20 prose pin materializes only as `www/assumptions.json` mechanic-text (formula + one limitation), mirroring the 2.0.0 M8/M10 documentation-only prose precedent. **MINOR bump** driven by the goldens.json predicate change (§12.6 "any extracted artifact"); params.json changes only in its emitted `modelVersion`. **[2026-07-07 follow-up — documentation-only, no behavior change; assumptions.json sha above updated accordingly]** Two internal review-process reference tags were removed from the §4 `reviewCapacityQueue` (M20) mechanic text — one in the `formula` field, one in a `limitation` — because they surface verbatim in the in-app "Every coefficient the engine uses" drawer. The technical content (integer clear-entry sojourn = ceil(reviewDwellDays); the neutral-identity clause) is unchanged. `params.json` and `goldens.json` remain byte-identical to the original 2.2.0 build; only the `assumptions.json` mechanic-text (and thus its sha) moved. |
