...
```json eigenorg:mechanic
{
  ...
  "limitations": [
    ...
    "reviewWaitDays reports the integer realized review sojourn ceil(reviewDwellDays) (the M20 fractional-dwell pin, mini-G2 #2: the sojourn is the integer clear-entry step gap, so a fractional reviewDwellDays is realized as its ceiling) and lags the instantaneous queue by the EMA smoothing.",
