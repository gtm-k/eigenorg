...
{ "id": "rbThroughputPlateau", "scenario": "reviewBottleneck",
  ...
  "rationale": "The Theory-of-Constraints invariant (Goldratt 'The Goal'): system throughput-to-done is capped by the slowest stage. Bottleneck done-rate ≈ reviewCapacity (1/step) while the unbounded control clears the full ~1.46/step completion stream, so the TRUE plateau ratio ≈ 0.68 (robust done-rate ratio; equivalently cumThroughput totals 58/85 = 0.68). Raising producer speed upstream cannot lift the plateau; only elevating the constraint can. CALIBRATION CAVEAT (2026-07-04): `throughput` is a per-step integer count (~1/step here) and this golden's metric is the p50 of that per-step count; at ~1/step the median cannot distinguish 1 from 1.46, so the measured evaluator value is 0.95 (NOT the true 0.68), and the 0.85 bound is NOT met by the p50 metric. The plateau itself is proven by rbQueueBuilds + rbWaitRises (a growing queue cleared at a fixed rate IS the plateau). Recommended fix (a §11.11 metric change, pending a user gate): switch this metric to cumThroughput@bottleneck / cumThroughput@unbounded at the final step (ratio 0.6824 measured), which a 0.85 ratioBelow bound clears with margin. (bound provisional - BLOCKED on the metric decision above; not satisfiable with the current p50-of-per-step-throughput metric.)" }
```
